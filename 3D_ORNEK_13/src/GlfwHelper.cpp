...
static void scrool_callback(GLFWwindow *window, double xOffset, double yOffset) {
	GlfwHelper *glfwHelper = static_cast<GlfwHelper*>(glfwGetWindowUserPointer(window));
	glfwHelper->getGlHelper()->smove(yOffset);
...
GlfwHelper::GlfwHelper() {
	...
	this->window = glfwCreateWindow(500, 440, "AB2015-3D Ornek 13", NULL, NULL);
